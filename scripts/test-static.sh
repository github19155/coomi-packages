...
required_paths=(
	...
	"scripts/test-termux-auth-prefix.sh"
...
termux_auth_test_file="$ROOT_DIR/scripts/test-termux-auth-prefix.sh"
grep -Fq 'git -C "$TEST_ROOT" apply --check' "$termux_auth_test_file"
grep -Fq 'termux-auth header still contains' "$termux_auth_test_file"
...
grep -Fq 'packages/termux-auth/build.sh' "$prepare_file"
grep -Fq 'coomi_termux_auth_header' "$prepare_file"
